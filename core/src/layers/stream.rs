//! `StreamLayer` mirrors the curried state of the Rue ownership-layer `stream`
...
use chia_sdk_driver::{DriverError, Layer, Puzzle, SpendContext};
...
impl StreamLayer {
    ...
    /// `mod_hash`; `owner_hash` becomes `recipient`). Used by `parse_puzzle`.
    ...
    pub fn launch_hints(&self) -> Vec<Bytes> {
        let owner: Bytes = self.recipient.into();
    ...
    /// Build the full `StreamCurry` wire type from this layer's parameters,
    /// injecting the constant `mod_hash` and mapping `recipient` → `owner_hash`.
    ...
    /// Allocate the curried inner puzzle into the context (private implementation).
    fn build_curried_puzzle(&self, ctx: &mut SpendContext) -> Result<NodePtr, DriverError> {
    ...
    ///
    /// Inherent method: Rust resolves it ahead of the identically-named
    /// `Layer::construct_puzzle`, so callers (incl. existing tests) need no
    /// `use chia_sdk_driver::Layer;`. The trait impl delegates to the same body.
    pub fn construct_puzzle(&self, ctx: &mut SpendContext) -> Result<NodePtr, DriverError> {
        self.build_curried_puzzle(ctx)
    }
